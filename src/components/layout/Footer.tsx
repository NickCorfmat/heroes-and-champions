...
export function Footer() {
  return (
    <footer className="w-full flex flex-row px-12 pt-8 pb-35 bg-black text-white">
      <div className="w-1/4 flex flex-col items-start">
        <Image
          src={logo}
          alt="Heroes & Champions logo"
          className="h-[70px] w-[170px] mb-5"
        />
      </div>

      {FOOTER_SECTIONS.map((section) => (
        <div key={section.heading} className="w-1/4 flex flex-col items-start gap-2">
          ...
          {section.items.map((item) => (
            <div key={item.label} className="flex flex-col gap-0.5">
              ...
              {"address" in item && (
                <>
                  <p className="text-white/45 text-xs">{item.address}</p>
